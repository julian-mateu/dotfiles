...
1. **`install.sh`** - Supports interactive, config-driven (`--config`), and profile (`--profile`) modes. In interactive mode, every tool prompts for confirmation. For config-driven mode, copy `dotfiles.conf.example` to `dotfiles.conf` and customize. To add new tools, register them in `register_all_tools()` and add the corresponding `INSTALL_*` variable to profiles and the example config.
